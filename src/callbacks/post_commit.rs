        ...
        match sah.action() {
            ...
            Action::CreateLink(create_link) => {
                ...
                let Ok(Some(_link_type)) = L::from_type(create_link.zome_index, create_link.link_type) else {
                    error!("CreateLink should have a LinkType. Could be a Link from a different zome: {} ({}) | {:?}", create_link.link_type.0, create_link.zome_index, create_link);
                    continue
                };
                /// Emit Link Signal
                ...
                let _ = emit_system_signal(SystemAttestation::PostCommitLink { link_type: create_link.link_type.0, is_delete: false, succeeded: res.is_ok() });
            },
            ///
            Action::DeleteLink(delete_link) => {
                ...
                let _ = emit_system_signal(SystemAttestation::PostCommitLink { link_type: create_link.link_type.0, is_delete: true, succeeded: res.is_ok() });
            ...
            Action::Create(_) => {
                ...
                let _ = emit_system_signal(SystemAttestation::PostCommitEntry { app_entry_type: variant_name, is_delete: false, succeeded: result.is_ok() });
                ///
                if let Err(e) = result {
            ...
            Action::Delete(delete) => {
                ...
                let type_variant = get_variant_from_index::<E>(app_entry_def.entry_index).unwrap();
                let variant_name = format!("{:?}", type_variant);
                let _ = emit_system_signal(SystemAttestation::PostCommitEntry { app_entry_type: variant_name, is_delete: true, succeeded: result.is_ok() });
                ///
                if let Err(e) = result {
